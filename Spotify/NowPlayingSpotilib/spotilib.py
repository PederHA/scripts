...
###Media Controls###
def hwcode(Media):
	hwcode = win32gui.MapVirtualKey(Media, 0)
	return hwcode

def next():
	win32gui.keybd_event(Media_Next, hwcode(Media_Next))
	
def previous():
	win32gui.keybd_event(Media_Previous, hwcode(Media_Previous))
	
def pause():
	win32gui.keybd_event(Media_Pause, hwcode(Media_Pause))
	
def play():
	win32gui.keybd_event(Media_Pause, hwcode(Media_Pause))
	
def mute():
	win32gui.keybd_event(Media_Mute, hwcode(Media_Mute))
